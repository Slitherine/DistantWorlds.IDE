...
public static partial class Helpers {
    ...
    public static nint ExtractHandle<T>(T m) where T : class
        => new GCHandle<T>(m).Value;

    /// <summary>
    /// Writes a string to an unmanaged buffer.
    /// </summary>
    /// <param name="str">String to write.</param>
    /// <param name="pBuffer">Pointer to a buffer to write the string to.</param>
    ...
    /// </returns>
    internal static unsafe int ExtractUnmanagedString(string? str, char* pBuffer, int bufferSize) {
        if (str is null)
            return 0;
        ...
        if (bufferSize < len || pBuffer is null)
            return -(len + 1); // +1 for null terminator

        chars.CopyTo(new(pBuffer, bufferSize));

        // only write null terminator if the buffer is large enough
        if (bufferSize <= len)
            return bufferSize;
    ...
    private static readonly RecyclableMemoryStreamManager RmsMgr = new(new RecyclableMemoryStreamManager.Options(
        /*4kB*/ 4 * 1024,
        /*2MB/4kB*/ 512,
        /*2MB*/ 2 * 1024 * 1024,
        /*8MB*/ 1 * 1024 * 1024,
    ...
    public static RecyclableMemoryStream GetRecyclableMemoryStream()
        => RmsMgr.GetStream();
