...
public class BundleManager {
  ...
  static BundleManager() {
    ...
    DefaultObjectDatabase = new ObjectDatabase(
...
#pragma warning disable VSTHRD200
  private static Task<string?> ResolveBundle(string name) {
    ...
    if (!BundleSourceFspStack.TryPeek(out var fsp))
    ...
    return Task.FromResult(found ? $"{fsp.RootPath}{name}.bundle" : null);
  }
#pragma warning restore VSTHRD200

  private static Dictionary<string, HashSet<string>> BundlePaths = new();

  private static ThreadLocal<ConcurrentStack<FileSystemProvider>> _BundleSourceFspStack = new(() => new());
  ...
  public static string LoadBundle(string bundlePath) {
    ...
    var fsp = new FileSystemProvider("/data/db/bundles/", directory);
    //VirtualFileSystem.RegisterProvider(fsp); // fsr FileSystemProvider's base calls this already
      ...
      */
      Content.FileProvider.ObjectDatabase.BundleBackend.LoadBundleFromUrl
        (bundleName, Content.FileProvider.ObjectDatabase.ContentIndexMap,
          $"/data/db/bundles/{bundleName}.bundle")
      ...
      foreach (var objId in Content.FileProvider.ObjectDatabase.BundleBackend.EnumerateObjects()) {
        // preload some data 
        try {
          Content.FileProvider.ObjectDatabase.BundleBackend.TryGetObjectLocation(objId, out var file, out _, out _);
  ...
  public static long GetObjectOffset(ObjectId? id, out long offsetEnd, out string? sourceDirOrBundlePath) {
    ...
    if (Content.FileProvider.ObjectDatabase.TryGetObjectLocation(id.Value, out sourceDirOrBundlePath, out var offset, out offsetEnd))
      return offset;
  ...
  private static readonly unsafe delegate *<ContentSerializerContext, SerializationStream, IContentSerializer, object, void>
  ...
  public static unsafe void ConvertToImages(Image img, bool includeMips,
    Action<SixLabors.ImageSharp.Image, int> withConverted, int mipLevel = 0, int maxMipLevel = -1) {
      ...
      if (compressionFormat is not
        ...
        if (compressionFormat is CompressionFormat.Bc6S or CompressionFormat.Bc6U) {
          ...
          if (mem2D.TryGetMemory(out var mem)) {
            var isImg = SixLabors.ImageSharp.Image.LoadPixelData<RgbVector>(mem.Span.AsBytes(), width, height);
